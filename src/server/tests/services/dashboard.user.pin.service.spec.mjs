...
describe('dashboard.user.services.mjs - PIN lifecycle', () => {
  ...
  describe('generatePickupPin - confirm borrow flow', () => {
    it('should generate a 6-digit PIN, set status pending, and return expiresAt 180s in the future', async () => {
    ...
    it('should return RESERVATION_NOT_FOUND when the reservation does not exist', async () => {
    ...
    it('should reuse the existing active PIN without regenerating', async () => {
      pool.query.mockImplementation(async (sql) => {
        if (sql.includes('WHERE borrow_id = $1 AND user_id = $2 AND status IN')) {
          return { rows: [{ borrow_id: BORROW_ID, user_id: USER_ID }] };
        }
    ...
    it('should return PIN_GENERATION_FAILED after 3 unique-violation attempts', async () => {
      vi.spyOn(Math, 'random').mockReturnValue(0.5);
      pool.query.mockImplementation(async (sql) => {
        if (sql.includes('WHERE borrow_id = $1 AND user_id = $2 AND status IN')) {
          return { rows: [{ borrow_id: BORROW_ID, user_id: USER_ID }] };
    ...
    it('should throw non-unique database errors', async () => {
      pool.query.mockImplementation(async (sql) => {
        if (sql.includes('WHERE borrow_id = $1 AND user_id = $2 AND status IN')) {
          return { rows: [{ borrow_id: BORROW_ID, user_id: USER_ID }] };
        }
  ...
  describe('generateReturnPin - confirm return flow', () => {
    it('should generate a PIN and set status pending_return for a borrowed book', async () => {
    ...
    it('should return BORROW_NOT_FOUND when the book is not currently borrowed', async () => {
      pool.query.mockResolvedValue({ rows: [] });
    ...
    it('should return INTERNAL_ERROR with 500 when a database failure occurs', async () => {
  ...
  describe('cleanupReturnPin', () => {
    it('should clear the return PIN and restore status borrowed when a row was updated', async () => {
    ...
    it('should return false when no row matched pending_return', async () => {
  ...
  describe('cleanupReservationPin', () => {
    it('should clear the pickup PIN and restore status reserved when a row was updated', async () => {
      pool.query.mockResolvedValue({ rowCount: 1 });
    ...
    it('should return false when no pending row matched', async () => {
      pool.query.mockResolvedValue({ rowCount: 0 });
