...
1. **Given** the current user has created groups in multiple statuses, **When** “Group I Created” loads, **Then** it shows persisted groups rather than mock data ordered by In Progress, Full, Upcoming, Completed, Cancelled, and Expired.
...
4. **Given** the host opens an In Progress, Completed, Cancelled, or Expired group, **When** details are shown, **Then** historical/session information remains viewable but group details are read-only.
...
1. **Given** the current user has participation records in several states, **When** “Group I Joined” loads, **Then** it shows persisted data rather than mock data ordered by Approved, Pending, Denied, then Expired; within a participation state, active In Progress, Full, and Upcoming sessions precede Completed, Cancelled, or Expired sessions.
...
5. **Given** the participation is Denied, or the associated group is In Progress, Completed, Cancelled, or Expired, **When** details are viewed, **Then** leave and cancel-request actions are unavailable.
...
- **FR-018**: Created groups MUST be ordered by status priority: In Progress, Full, Upcoming, Completed, Cancelled, Expired.
...
- **FR-022**: Active or actionable records MUST appear above historical records so users see attendance, capacity, and approval work first. Created cards MUST be dimmed only for Completed, Cancelled, and Expired groups. Joined cards MUST remain visually active for actionable Approved or Pending participation and MUST be dimmed for Denied or Expired participation, as well as when the associated group is Completed, Cancelled, or Expired.
...
- **FR-048**: Existing Study Together layout, filtering, group-detail, and join-request interaction patterns MUST be preserved unless a change is required to represent real lifecycle state accurately. No user-facing Sort By control is shown; discovery uses the authoritative ordering in FR-058.
...
- **FR-060**: A Pending card in Study Together MUST expose the same localized, confirmed Cancel Request behavior as Group I Joined and MUST converge with the host queue through the existing realtime refresh path.
...
- **FR-062**: A successful invitation MUST create exactly one `group_request` row with `type = invite` and `status = pending`, then send an email using the existing configured mail transport. Hosts cannot invite themselves, unknown emails are rejected, and an active Pending or Approved relationship prevents duplicate invitations.
- **FR-063**: Pending Study Group invitations MUST appear as one typed notification within the authenticated user's existing system-wide notification bell, with the shared unread count. The notification panel header and empty state MUST remain generic so other library features can add notification types later. Selecting a Study Group invitation MUST open its localized detail popup with Accept and Deny actions and identify the creator who performed the invitation using their profile picture when available, username, and email.
...
- **FR-065**: Accepting an invitation MUST atomically change it to Approved and increment capacity exactly once. After success, the user MUST be routed to `/dashboard/user/yourstudygroups`, the Joined tab MUST be selected, and a localized joined-success notification MUST be shown.
- **FR-066**: Denying an invitation MUST change it to Denied, remove it from the notification list, leave member count unchanged, and close the invitation popup. Invite denial MUST NOT trigger the 30-minute join-request denial cooldown.
...
- **FR-069**: Created and Joined dashboard status controls MUST support selecting any combination of statuses. Results MUST match any selected status, selecting a selected status again MUST remove it, and activating All Status MUST clear every selection and show all records.
...
- **BR-013**: `reserve_room.status` is limited to `pending`, `reserved`, and `used` by the authoritative schema. Cancelling an eligible reservation or dissolving its Study Group permanently deletes that reservation; the linked group and requests are removed by cascade and no Cancelled reservation history is retained. Legacy `study_group.status = cancelled` rows may be read only as display fixtures and are never created by the real dissolve flow.
...
2. They may combine subject, date, time, one-or-both branch, and all-or-selected-room filters populated from current facility information.
