...
| genres | array[string] | List of categories/genres |
...
| embedding | vector | Vector representation of `description` for pgvector similarity matching |

### SearchHistory
Represents the logs of queries and filters run by authenticated (logged-in) users.

| Field | Type | Description |
|-------|------|-------------|
| id | string | Unique log entry ID |
| userId | string | Reference to User.id (logged-in user) |
| searchContent | string | Composed text logging search query and active filter tags |
| searchMode | string | The mode of search: "standard" or "semantic" |
| filters | object | JSON object of applied filters (genres, branches, availableOnly, publicationDate) |
| clickedBookIds | array[string] | List of book IDs clicked from this search's results (intent clicks) |
...
- **SearchHistory (N) -> (N) Book**: A search history record can log multiple clicked book IDs (intent clicks) referencing the Book entity.
